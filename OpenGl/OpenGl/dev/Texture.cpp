...
Texture::Texture()
{
	...
	pNode->set("uninitialized", Name::NOT_INITIALIZED, textureID, GL_NEAREST, GL_NEAREST, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE);
	privAddToFront(pNode, active);*/
}
...
GLuint Texture::Find( const Texture::Name _name)
{
	...
	if (pNode == nullptr)
	{
		pNode = (TextureNode *)pTextMan->active;
	}
