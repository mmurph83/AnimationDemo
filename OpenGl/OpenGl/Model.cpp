...
Model::Model(unsigned char* vertData, unsigned char* triData, const unsigned int totalVerts, const unsigned int totalTris)
{
	Vert_xyzuvn *verts = (Vert_xyzuvn*)(vertData);
	Tri_index *triList = (Tri_index*)(triData);
	this->numTris = totalTris / sizeof(Tri_index);
	this->numVerts = totalVerts / sizeof(Vert_xyzuvn);
...
Model::Model(BaseModelType modelType)
{
	privGenSphere();
	...
	case BaseModelType::Cube:
	{
		...
		this->numVerts = sizeof(cubeData) / sizeof(Vert_xyzuvn);
		this->numTris = 12;
	...
	case BaseModelType::Sphere:
	{
		...
		this->numVerts = sizeof(pVerts) / sizeof(Vert_xyzuvn);
		this->numTris = sizeof(triList) / sizeof(Tri_index);
	...
	case BaseModelType::Pyramid:
	{
		...
		this->numVerts = sizeof(pyramidData) / sizeof(Vert_xyzuvn);
		this->numTris = (sizeof(triList) / sizeof(Tri_index));
	...
	case BaseModelType::Diamond:
	{
		...
		{
			// Triangle 0
			{ 0.0f,  0.25f, 0.0f,        0.5f,  1.0f, 0.0f,  0.25f, 0.0f },

			{ 0.25f, -0.25f, -0.25f,     1.0f, 1.0f, 0.25f, -0.25f, -0.25f },
			{ 0.25f, -0.25f, 0.25f,	     1.0f, 0.0f,  0.25f, -0.25f, 0.25f },
			{ -0.25f, -0.25f, 0.25f,     0.0f, 0.0f, -0.25f, -0.25f, 0.25f },
			{ -0.25f, -0.25f, -0.25f,    1.0f, 0.0f, -0.25f, -0.25f, -0.25f },
			{ 0.0f,  -0.75f, 0.0f,       0.5f,  1.0f, 0.0f,  -0.75f, 0.0f },
		};
		this->numVerts = sizeof(pyramidData) / sizeof(Vert_xyzuvn);
		this->numTris = sizeof(triList)/sizeof(Tri_index);
...
{
	bindVAO();
	glDrawElements(GL_TRIANGLES, 3 * getNumTris(), GL_UNSIGNED_INT, 0);
}
...
void Model::privGenSphere()
{
	const int horizontal = 5;
	const int vertical = 1;
	int temp = 0;
	Vert_xyzuvn  sphereData[horizontal*vertical + 2];
	Vect rotVect(0.0f, 1.0f, 0.0f);
	Matrix rotX(ROT_X, 3.14f / (float)(vertical + 1));
	Matrix rotY(ROT_Y, 3.14f / (float)(horizontal));
	rotVect *= rotX;
	for (int j = 0; j < vertical; j++)
	{
		for (int i = 0; i < horizontal; i++)
		{
			sphereData[temp++] = { rotVect[x],rotVect[y],rotVect[z],0.0f,0.0f };
			rotVect *= rotY;
		}
		rotVect *= rotX;
	}
	Vert_xyzuvn  sphereVertData[(horizontal*vertical + 2)*3];
	temp = 0;
	for (int i = 0; i < horizontal - 1; i++)
	{
		sphereVertData[temp++] = sphereData[i];
		sphereVertData[temp++] = sphereData[i + 1];
		sphereVertData[temp++] = { 0.0f,1.0f,0.0f,0.0f,0.0f };
	}
}
